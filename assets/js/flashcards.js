    ...
    renderGrid() {
        ...
        this.allCards.forEach(card => {
            ...
            const imgHTML = card.image_url
                ? `<div class="h-32 bg-gray-100 rounded-lg overflow-hidden mb-3"><img src="${this.escapeAttr(card.image_url)}" class="w-full h-full object-cover" alt="" loading="lazy" onerror="if(!this.dataset.tried){this.dataset.tried='1'; this.src='uploads/flashcards/${this.escapeAttr(card.image_path || '')}';}"></div>`
                : '';
    ...
    renderTable() {
        ...
        this.allCards.forEach(card => {
            ...
            const thumb = card.image_url
                ? `<img src="${this.escapeAttr(card.image_url)}" class="w-10 h-10 rounded-lg object-cover" loading="lazy" onerror="if(!this.dataset.tried){this.dataset.tried='1'; this.src='uploads/flashcards/${this.escapeAttr(card.image_path || '')}';}">`
